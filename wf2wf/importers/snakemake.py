...
def to_workflow(path: str | Path, **opts: Any) -> Workflow:
    ...
    sm_cli_args = ['snakemake', '--snakefile', str(snakefile_path), '--cores', str(cores), '--quiet']
    if workdir:
    ...
    try:
        dag_process = subprocess.run(dag_cmd, capture_output=True, text=True, check=True)
        ...
        raise RuntimeError(f"'snakemake --dag' failed (exit code {e.returncode}). Stderr:\n{e.stderr}")
    ...
    try:
        dryrun_process = subprocess.run(dryrun_cmd, capture_output=True, text=True, check=True)
        ...
        print(f"WARNING: 'snakemake --dry-run' failed (exit code {e.returncode}). Parsing may be incomplete.")
    ...
    for i, (jobid, rule_name) in enumerate(job_labels.items()):
        ...
        resources = ResourceSpec(
            ...
            extra={k: v for k, v in merged_resources.items() 
                   if k not in {'threads', 'cpu', 'mem_mb', 'mem', 'disk_mb', 'disk_gb', 'gpu', 'gpu_mem_mb'}}
        )

        # Build environment
        environment = EnvironmentSpec(
            conda=template.get('conda'),
    ...
    if verbose:
        print(f"INFO: Successfully created workflow with {len(wf.tasks)} tasks and {len(wf.edges)} edges")
...
def _workflow_to_dag_info(wf: Workflow) -> Dict[str, Any]:
    ...
    for task in wf.tasks.values():
        ...
        job_dict = {
            ...
            'resources': {
                **asdict(task.resources),
                **task.resources.extra
            },
...
def _parse_snakefile_for_rules(snakefile_path, debug=False):
    ...
    for i, rule_info in enumerate(rule_starts):
        ...
        for directive in ['input', 'output', 'log', 'conda', 'container', 'shell', 'script']:
            # This regex handles single/double quotes, raw strings, and allows for newlines
            pattern = re.compile(fr"^\s*{directive}:\s*(?:['\"](.*?)(?<!\\)['\"]|r['\"](.*?)(?<!\\)['\"])", re.S | re.M)
        ...
        for line in body_lines: # Iterate over the lines of the body
            ...
            if in_run_block and line and not line.startswith((' ', '\t')) and stripped_line:
        ...
        for line in body_lines:
            ...
            if in_resources_block and line and not line.startswith((' ', '\t')) and stripped_line:
                if ':' in stripped_line and not stripped_line.startswith('#'):
                    in_resources_block = False
...
def _parse_dryrun_output(dryrun_output, debug=False):
    ...
    def format_job(data):
        ...
        job_info = {
            ...
            'wildcards_dict': dict(item.split('=', 1) for item in data.get('wildcards', '').split(', ')) if data.get('wildcards') else {},
            'resources': resources,
    ...
    for line in dryrun_output.splitlines():
        ...
        if re.match(r'^\[.+\]$', line) or '...' in line or 'Building DAG' in line or 'Job stats' in line or 'job count' in line or '---' in line or 'total' in line or 'host:' in line:
            continue
