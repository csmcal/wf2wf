...
from wf2wf.core import (
    ...
    ParameterSpec, ProvenanceSpec, DocumentationSpec
)
...
def to_workflow(path: str | Path, **opts: Any) -> Workflow:
    ...
    try:
        ...
        if verbose:
            print(f"Successfully imported Galaxy workflow with {len(workflow.tasks)} tasks")
...
def _convert_galaxy_to_workflow(galaxy_doc: Dict[str, Any], galaxy_path: Path,
    ...
    for step_id, step_data in steps.items():
        ...
        if step_type == 'data_input':
            ...
            input_step = _convert_galaxy_input_step(step_id, step_data, preserve_metadata)
...
def _convert_galaxy_input_step(step_id: str, step_data: Dict[str, Any], 
                              preserve_metadata: bool = True) -> ParameterSpec:
...
def _convert_galaxy_tool_step(step_id: str, step_data: Dict[str, Any],
...
def _extract_galaxy_tool_inputs(tool_state: Dict[str, Any], 
                               step_data: Dict[str, Any]) -> List[ParameterSpec]:
...
def _extract_galaxy_connections(step_id: str, step_data: Dict[str, Any], 
                               all_steps: Dict[str, Any]) -> List[Edge]:
    ...
    for input_name, connection in input_connections.items():
        if isinstance(connection, dict):
            ...
            if source_step and source_step in all_steps:
                edges.append(Edge(parent=f"step_{source_step}", child=f"step_{step_id}"))
...
def _extract_galaxy_outputs(steps: Dict[str, Any], tasks: Dict[str, Task]) -> List[ParameterSpec]:
...
def _extract_galaxy_documentation(galaxy_doc: Dict[str, Any]) -> Optional[DocumentationSpec]:
