...
from wf2wf.core import (
    Workflow, Task, Edge, ResourceSpec, EnvironmentSpec,
    ParameterSpec, ScatterSpec, RequirementSpec, ProvenanceSpec,
    DocumentationSpec, TypeSpec, FileSpec
)
from wf2wf.loss import apply as loss_apply, LossEntry
...
def to_workflow(path: str | Path, **opts: Any) -> Workflow:
    ...
    try:
        ...
        if '$graph' in cwl_doc:
            ...
            root_wf_obj = next((o for o in graph_list if o.get('class') == 'Workflow'), None)
        ...
        if verbose:
            print(f"Successfully imported CWL workflow with {len(workflow.tasks)} tasks")
            print(f"Enhanced features: {len(workflow.requirements)} requirements, "
        ...
        if loss_path.exists():
            try:
                with open(loss_path) as fh:
                    doc = json.load(fh)
                    if doc.get('source_checksum') and doc['source_checksum'] != compute_checksum(workflow):
                        if verbose:
                            print(f"Skipping loss-map from {loss_path} (checksum mismatch)")
                    else:
                        entries = doc.get('entries', [])
                        workflow.loss_map.extend(entries)
                        loss_apply(workflow, entries)  # best-effort reinjection
                        if verbose:
                            print(f"Applied {len(entries)} loss-map entries from {loss_path}")
...
def _parse_cwl_workflow(cwl_doc: Dict[str, Any], cwl_path: Path, 
                       preserve_metadata: bool = True,
                       extract_provenance: bool = True,
                       verbose: bool = False, debug: bool = False,
                       graph_objects: Optional[Dict[str, Any]] = None) -> Workflow:
    ...
    if 'intent' in cwl_doc:
        workflow.intent = cwl_doc['intent'] if isinstance(cwl_doc['intent'], list) else [cwl_doc['intent']]
    ...
    if not steps:
        ...
        if verbose:
            print("Warning: CWL workflow has no steps defined - creating empty workflow")
        return workflow
...
def _parse_cwl_step(step_name: str, step_def: Dict[str, Any], cwl_path: Path,
                   verbose: bool = False, debug: bool = False) -> tuple[Task, List[Edge]]:
...
def _load_tool_definition(run_ref: Union[str, Dict[str, Any]], cwl_path: Path,
...
def _convert_tool_to_workflow(tool_def: Dict[str, Any], cwl_path: Path, 
                             preserve_metadata: bool = True,
                             verbose: bool = False) -> Workflow:
...
def _parse_with_cwltool(cwl_path: Path, fallback_workflow: Workflow, 
                       verbose: bool = False) -> Workflow:
...
def _extract_provenance_spec(cwl_doc: Dict[str, Any]) -> Optional[ProvenanceSpec]:
    ...
    return provenance if any([
        provenance.authors, provenance.version, provenance.license,
        provenance.doi, provenance.keywords, provenance.extras
    ]) else None
...
def _extract_documentation_spec(cwl_doc: Dict[str, Any]) -> Optional[DocumentationSpec]:
    ...
    return doc_spec if any([
        doc_spec.description, doc_spec.label, doc_spec.doc, doc_spec.intent
    ]) else None


def _parse_parameter_specs(params: Union[Dict, List], param_type: str) -> List[ParameterSpec]:
    ...
    if isinstance(params, dict):
        ...
        for param_def in params:
            if isinstance(param_def, dict) and 'id' in param_def:
                param_spec = _parse_single_parameter_spec(param_def['id'], param_def, param_type)
                if param_spec:
                    param_specs.append(param_spec)
...
def _parse_single_parameter_spec(param_id: str, param_def: Any, param_type: str) -> Optional[ParameterSpec]:
...
def _parse_cwl_step_enhanced(step_name: str, step_def: Dict[str, Any], cwl_path: Path,
                           preserve_metadata: bool = True,
                           verbose: bool = False, debug: bool = False,
                           graph_objects: Optional[Dict[str, Any]] = None) -> tuple[Task, List[Edge]]:
    ...
    # Load the tool definition
    tool_def = _load_tool_definition(run_ref, cwl_path, graph_objects=graph_objects, verbose=verbose)
...
def _extract_resource_requirements_enhanced(tool_def: Dict[str, Any], 
                                          step_def: Dict[str, Any]) -> ResourceSpec:
    ...
    for req in all_requirements:
        ...
        if req_class == 'ResourceRequirement':
            ...
            if 'coresMax' in req:
                resources.cpu = max(resources.cpu, int(req['coresMax']))  # Use coresMax for CPU
            ...
            if 'ramMin' in req:
                ...
                if isinstance(ram_min, str) and ram_min.endswith('G'):
                    resources.mem_mb = max(resources.mem_mb, int(float(ram_min[:-1]) * 1024))
            ...
            if 'ramMax' in req:
                ...
                if isinstance(ram_max, str) and ram_max.endswith('G'):
                    resources.mem_mb = max(resources.mem_mb, int(float(ram_max[:-1]) * 1024))
...
def _extract_environment_spec_enhanced(tool_def: Dict[str, Any], 
                                     step_def: Dict[str, Any]) -> EnvironmentSpec:
            ...
            if packages:
                ...
                for pkg in packages:
                    if isinstance(pkg, dict):
                        ...
                        pkg_version = pkg.get('version', [''])[0] if pkg.get('version') else ''
                        if pkg_name:
                            if pkg_version:
                                conda_spec['dependencies'].append(f"{pkg_name}={pkg_version}")
                            else:
...
def _parse_step_dependencies_enhanced(step_name: str, step_def: Dict[str, Any], 
            ...
            for src in sources:
                if isinstance(src, str) and '/' in src:
                    ...
                    if parent_step != step_name:  # Avoid self-dependencies
                        edges.append(Edge(parent=parent_step, child=step_name))
                        if verbose:
                            print(f"  Found enhanced dependency: {parent_step} -> {step_name}")
...
def _extract_inputs_legacy(step_def: Dict[str, Any], tool_def: Dict[str, Any]) -> List[str]:
...
def _extract_outputs_legacy(step_def: Dict[str, Any], tool_def: Dict[str, Any]) -> List[str]:
