...
def to_workflow(path: str | Path, **opts: Any) -> Workflow:
    ...
    try:
        ...
        if verbose:
            print(f"Successfully imported WDL workflow with {len(workflow.tasks)} tasks")
...
def _parse_wdl_document(content: str, wdl_path: Path, debug: bool = False) -> Dict[str, Any]:
    ...
    # Extract imports
    import_matches = re.finditer(r'import\s+"([^"]+)"(?:\s+as\s+(\w+))?', content, re.IGNORECASE)
    ...
    for match in workflow_starts:
        ...
        doc['workflows'][workflow_name] = _parse_wdl_workflow(workflow_body, workflow_name, debug=debug)
...
def _parse_wdl_task(task_body: str, task_name: str, debug: bool = False) -> Dict[str, Any]:
    ...
    command_match = re.search(r'command\s*(?:<<<|{)([^}]*?)(?:>>>|})', task_body, re.DOTALL)
    if command_match:
...
def _parse_wdl_workflow(workflow_body: str, workflow_name: str, debug: bool = False) -> Dict[str, Any]:
    ...
    param_meta_match = re.search(r'parameter_meta\s*\{([^}]*)\}', workflow_body, re.DOTALL)
    if param_meta_match:
...
def _parse_wdl_call_inputs(inputs_text: str) -> Dict[str, str]:
    ...
    input_assignments = [line.strip() for line in cleaned_text.replace('\n', ',').split(',') if line.strip()]
...
def _convert_wdl_to_workflow(wdl_doc: Dict[str, Any], wdl_path: Path,
                            preserve_metadata: bool = True,
                            verbose: bool = False, debug: bool = False) -> Workflow:
    ...
    if preserve_metadata:
        ...
        if main_workflow.get('meta') or main_workflow.get('parameter_meta'):
            workflow.documentation = _extract_wdl_documentation(
                main_workflow.get('meta', {}),
                main_workflow.get('parameter_meta', {})
            )
...
def _convert_wdl_task_to_ir(wdl_task: Dict[str, Any], task_id: str, call: Dict[str, Any],
                           preserve_metadata: bool = True, verbose: bool = False) -> Task:
...
def _convert_wdl_inputs(wdl_inputs: Dict[str, Any]) -> List[ParameterSpec]:
    ...
    for param_name, param_def in wdl_inputs.items():
        ...
        if isinstance(default_val, str) and len(default_val) >= 2 and (
            (default_val.startswith('"') and default_val.endswith('"')) or (default_val.startswith("'") and default_val.endswith("'"))):
...
def _convert_wdl_scatter(scatter_expr: str) -> ScatterSpec:
    ...
    if scatter_match:
        item_var = scatter_match.group(1)
        collection_var = scatter_match.group(2)
        
        return ScatterSpec(
            scatter=[collection_var],
...
def _extract_wdl_dependencies(call: Dict[str, Any], call_alias: str, 
                             all_calls: List[Dict[str, Any]]) -> List[Edge]:
...
def _extract_wdl_documentation(meta: Dict[str, Any], 
                              parameter_meta: Dict[str, Any]) -> Optional[DocumentationSpec]:
