...
def to_workflow(path: str | Path, **opts: Any) -> Workflow:
        ...
        if job_info.get('inline_submit'):
            ...
            if str(submit_file) not in submit_files:
                if submit_file.exists():
                    submit_files[str(submit_file)] = _parse_submit_file(submit_file, debug=debug)
                else:
                    if verbose:
    ...
    if verbose:
        print(f"Created workflow '{wf.name}' with {len(wf.tasks)} tasks and {len(wf.edges)} dependencies")
    
    return wf


def _parse_dag_file(content: str, debug: bool = False) -> Tuple[Dict[str, Dict], List[Tuple[str, str]], Dict[str, str], Dict[str, Any]]:
            ...
            if job_match:
                ...
                if job_spec.startswith('{'):
                    ...
                    if job_spec == '{':
                        ...
                        inline_content = [job_spec[1:-1].strip()] if job_spec.endswith('}') else []
                    ...
                    if debug:
                        print(f"DEBUG: Parsed inline submit for {job_name}: {len(inline_content)} lines")
                else:
...
def _parse_submit_file(submit_path: Path, debug: bool = False) -> Dict[str, Any]:
    ...
    if debug:
        ...
        print(f"  resources: cpu={submit_info['resources'].cpu}, mem={submit_info['resources'].mem_mb}MB")
    
    return submit_info


def _create_task_from_job(job_name: str, job_info: Dict, submit_info: Dict, dag_dir: Path) -> Task:
    ...
    if submit_info.get('executable'):
        ...
        if exec_path.exists() and exec_path.suffix == '.sh':
            ...
            try:
                ...
                script = executable
                command = f"{executable} {arguments}".strip() if arguments else executable
        else:
            # Regular executable
