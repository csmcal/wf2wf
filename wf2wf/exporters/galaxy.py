...
def from_workflow(wf: Workflow, out_file: str | Path, **opts: Any) -> None:
    ...
    try:
        ...
        if wf.intent:
            loss_record("/intent", "intent", wf.intent, "Galaxy workflow schema lacks intent field", "user")

        for task in wf.tasks.values():
            if task.scatter:
                loss_record(f"/tasks/{task.id}/scatter", "scatter", task.scatter.scatter, "Galaxy lacks scatter construct", "user")
            if task.when:
                loss_record(f"/tasks/{task.id}/when", "when", task.when, "Conditional execution not preserved in Galaxy", "user")
        ...
        loss_write(output_path.with_suffix('.loss.json'), target_engine='galaxy', source_checksum=compute_checksum(wf))
...
def _generate_galaxy_workflow(wf: Workflow, galaxy_version: str,
                             preserve_metadata: bool = True, verbose: bool = False) -> Dict[str, Any]:
    ...
    if preserve_metadata and wf.provenance and wf.provenance.authors:
        galaxy_doc["creator"] = [author.get('name', 'Unknown') for author in wf.provenance.authors]
...
def _generate_galaxy_input_step(input_param: ParameterSpec, step_id: int) -> Dict[str, Any]:
    ...
    step = {
        ...
        "outputs": [
            {
                "name": "output",
                "type": _convert_ir_type_to_galaxy(input_param.type)
            }
        ],
        "position": {
            "left": 10,
...
def _generate_galaxy_tool_step(task: Task, step_id: int, workflow: Workflow, 
    ...
    input_connections = _generate_galaxy_input_connections(task, workflow, task_to_step_id)
...
def _generate_galaxy_input_connections(task: Task, workflow: Workflow, 
                                     task_to_step_id: Dict[str, int]) -> Dict[str, Any]:
    ...
    for input_param in workflow.inputs:
        ...
        for task_input in task.inputs:
            if task_input.id == input_param.id or task_input.id.endswith(input_param.id):
