...
def from_workflow(wf: Workflow, out_file: str | Path, **opts: Any):
    ...
    if wf.intent:
        loss_record("/intent", "intent", wf.intent, "Snakemake has no ontology intent field", "user")

    for task in wf.tasks.values():
        if task.scatter:
            loss_record(f"/tasks/{task.id}/scatter", "scatter", task.scatter.scatter, "Native scatter not supported in Snakemake", "user")
        if task.when:
            loss_record(f"/tasks/{task.id}/when", "when", task.when, "Conditional execution not directly supported in Snakemake", "user")
        if task.resources.gpu or task.resources.gpu_mem_mb:
            loss_record(f"/tasks/{task.id}/resources/gpu", "gpu", task.resources.gpu, "GPU scheduling not expressible in Snakemake", "user")

        # Secondary files on outputs lost
        for p in task.outputs:
            if isinstance(p, ParameterSpec) and p.secondary_files:
                loss_record(f"/tasks/{task.id}/outputs/{p.id}/secondary_files", "secondary_files", p.secondary_files, "Secondary files concept missing in Snakemake", "user")
    ...
    loss_write(out_path.with_suffix('.loss.json'), target_engine='snakemake', source_checksum=compute_checksum(wf))
    wf.loss_map = as_list()
...
def _generate_config_section(wf: Workflow, config_file: Optional[Path], out_path: Path) -> List[str]:
...
def _has_non_default_resources(resources: ResourceSpec) -> bool:
    ...
    return (
        resources.cpu != 1 or
        resources.mem_mb > 0 or
        resources.disk_mb > 0 or
        resources.gpu > 0 or
        resources.gpu_mem_mb > 0 or
        resources.time_s > 0 or
        resources.threads != 1 or
        resources.extra
    )
...
def _generate_resource_spec(resources: ResourceSpec) -> List[str]:
    ...
    if resources.time_s > 0:
        ...
        if resources.time_s >= 3600:
            ...
            lines.append(f"        runtime={int(hours * 60)},")  # Snakemake uses minutes
        else:
