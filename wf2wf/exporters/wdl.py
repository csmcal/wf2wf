...
def from_workflow(wf: Workflow, out_file: str | Path, **opts: Any) -> None:
    ...
    try:
        ...
        if wf.intent:
            loss_record("/intent", "intent", wf.intent, "WDL spec has no intent field", "user")

        for task in wf.tasks.values():
            if task.when:
                loss_record(f"/tasks/{task.id}/when", "when", task.when, "Conditional when unsupported in WDL 1.0", "user")
            if task.resources.gpu or getattr(task.resources, "gpu_mem_mb", None):
                loss_record(f"/tasks/{task.id}/resources/gpu", "gpu", task.resources.gpu, "GPU resources not first-class in WDL runtime", "user")
        ...
        loss_write(output_path.with_suffix('.loss.json'), target_engine='wdl', source_checksum=compute_checksum(wf))
...
def _generate_wdl_workflow(wf: Workflow, wdl_version: str, 
                          preserve_metadata: bool = True, verbose: bool = False) -> str:
    ...
    workflow_wdl = _generate_wdl_workflow_definition(wf, preserve_metadata=preserve_metadata)
    ...
    if task.inputs:
        ...
        for param in task.inputs:
            ...
            if param.default is not None:
                lines.append(f"        {wdl_type} {param.id} = {_format_wdl_value(param.default)}")
            else:
...
def _generate_wdl_workflow_definition(wf: Workflow, preserve_metadata: bool = True) -> str:
    ...
    if wf.outputs:
        ...
        for param in wf.outputs:
            ...
            if producing_task:
                lines.append(f"        {wdl_type} {param.id} = {producing_task}.{param.id}")
            else:
...
def _generate_wdl_runtime(resources: ResourceSpec, environment: EnvironmentSpec) -> Dict[str, str]:
