...
from wf2wf.loss import reset as loss_reset, write as loss_write, record as loss_record, prepare, as_list, compute_checksum
...
def from_workflow(
    ...
    **opts: Any,
    ...
    for task in wf.tasks.values():
        if task.scatter:
            loss_record(f"/tasks/{task.id}/scatter", "scatter", task.scatter.scatter, "DAGMan has no scatter primitive", "user")
        if task.when:
            loss_record(f"/tasks/{task.id}/when", "when", task.when, "Conditional when lost in DAGMan", "user")
    ...
    loss_write(out_path.with_suffix('.loss.json'), target_engine='dagman', source_checksum=compute_checksum(wf))
    wf.loss_map = as_list()
...
def prepare_conda_setup_jobs(dag_info, conda_prefix, verbose=False, debug=False):
    ...
    for job in dag_info['jobs'].values():
        ...
        if env_spec and Path(env_spec).is_file() and (env_spec.endswith(".yaml") or env_spec.endswith(".yml")):
            ...
            if env_path not in conda_envs:
                try:
                    ...
                    if verbose:
                        print(f"WARNING: Could not find conda env file {env_path}. Cannot create setup job.")

    dag_info['conda_envs'] = conda_envs
    if verbose:
        if conda_envs:
            print(f"INFO: Identified {len(conda_envs)} unique conda environments to be created.")
    ...
    if debug:
        print(f"DEBUG: conda_envs structure: {json.dumps(conda_envs, indent=2, default=str)}")
...
def build_and_push_docker_images(dag_info, docker_registry, verbose=False, debug=False):
    ...
    for original_yaml_path, env_info in conda_envs.items():
        ...
        repo_name = _sanitize_condor_job_name(Path(dag_info.get('snakefile', 'workflow')).stem)
        ...
        try:
            if verbose: print(f"  Checking for remote manifest...")
            # Use docker manifest inspect to check remote without pulling
            result = subprocess.run(["docker", "manifest", "inspect", full_image_url], 
                                  check=True, capture_output=True, text=True)
            if debug:
                print(f"DEBUG: Manifest found, image exists")
            print("  ✔ Image already exists in registry. Skipping build.")
            continue
        except subprocess.CalledProcessError as e:
            if verbose: print("  Image not found in registry. Proceeding with build and push.")
            if debug: print(f"DEBUG: Manifest check failed with code {e.returncode}")
        ...
        with tempfile.TemporaryDirectory() as build_context:
            ...
            # Generate Dockerfile
            dockerfile_path = _generate_dockerfile(original_yaml_path, build_context_path)
            ...
            try:
                ...
                proc = subprocess.Popen(build_cmd, cwd=build_context_path, 
                ...
                if proc.returncode != 0:
                    print(f"  ✗ ERROR: Docker build failed for {full_image_url}. See output above.")
                    sys.exit(1)
                print("  ✔ Build successful.")
            except Exception as e:
                print(f"  ✗ ERROR: An unexpected error occurred during docker build: {e}")
                if debug:
                    import traceback
            ...
            try:
                ...
                proc = subprocess.Popen(push_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
                if verbose or debug:
                     for line in iter(proc.stdout.readline, ''):
                        prefix = "DEBUG: " if debug else "    "
                        print(f"{prefix}{line.strip()}")
                ...
                if proc.returncode != 0:
                    ...
                    print(f"  Please ensure you are logged in to '{docker_registry}' and have push permissions.")
                    sys.exit(1)
                print("  ✔ Push successful.")
            except Exception as e:
                print(f"  ✗ ERROR: An unexpected error occurred during docker push: {e}")
                if debug:
                    import traceback
                    print(f"DEBUG: Full traceback:\n{traceback.format_exc()}")
                sys.exit(1)
...
def convert_docker_to_apptainer(dag_info, sif_dir, verbose=False, debug=False):
    ...
    if not apptainer_cmd:
        print("WARNING: 'apptainer' or 'singularity' command not found. Skipping conversion.")
        return
    ...
    for env_info in conda_envs.values():
        ...
        try:
            ...
            build_cmd = [apptainer_cmd, "build", "--force", str(target_sif_path), f"docker://{docker_image_url}"]
            if debug:
...
def generate_job_scripts(dag_info, output_dir="scripts", verbose=False, debug=False):
    ...
    for original_yaml_path, env_info in conda_envs.items():
        ...
        script_paths[setup_job_name] = str(setup_script_path) # Add to script paths for DAG writing

        if verbose:
            print(f"  Generating conda setup script for {original_yaml_path.name} at {setup_script_path}")

        with open(setup_script_path, 'w') as f:
            f.write("#!/bin/bash\n")
            f.write("set -euo pipefail\n\n")
            f.write(f"# snake2dagman: Conda environment setup script for {original_yaml_path.name}\n")
            f.write(f"# This script creates the environment in a shared location if it doesn't already exist.\n\n")
            f.write(f"ENV_PREFIX=\"{env_info['install_path']}\"\n")
            f.write(f"ENV_YAML=\"{original_yaml_path}\"\n\n")
            
            f.write("# Check if environment already exists to make this job idempotent\n")
            f.write("if [ -f \"${ENV_PREFIX}/bin/python\" ]; then\n")
            f.write("    echo \"Conda environment already exists at ${ENV_PREFIX}. Nothing to do.\"\n")
            f.write("    exit 0\n")
            f.write("fi\n\n")

            f.write("echo \"Creating conda environment at ${ENV_PREFIX}...\"\n")
            f.write("# Use --force to overwrite a potentially corrupted/incomplete environment\n")
            f.write("conda env create --prefix \"${ENV_PREFIX}\" --file \"${ENV_YAML}\" --force\n")
    ...
    for job_uid, job_details in dag_info['jobs'].items():
        ...
        if verbose:
            print(f"  Generating script for job UID {job_uid} (Rule: {job_details['rule_name']}) at {sh_script_path}")
        
        if debug:
            print(f"DEBUG: Generating script for job {job_uid}, Details: {json.dumps(job_details, indent=4, default=str)}")
        ...
        if shell_cmd:
            ...
            interpreter = ""
            if script_file.lower().endswith(".py"): interpreter = "python"
            elif script_file.lower().endswith(".r") or script_file.lower().endswith(".R"): interpreter = "Rscript"
            elif script_file.lower().endswith(".sh"): interpreter = "bash"
            ...
            if interpreter:
                ...
                with open(sh_script_path, 'a') as f: # Append warning to the top
                    f.write(f"# WARNING: Could not determine interpreter for script: {script_file}\n")
            ...
            if run_code is not None:
                ...
                generate_run_block_script(py_script_path, job_uid, job_details, run_code, dag_info.get('config', {}), debug=debug)
                core_command = f"python {py_script_path}"
            else:
                core_command = f"echo \"'run:' block for {job_uid} was empty. No action taken.\""
            ...
            core_command = f"echo 'Job placeholder for {job_uid} - no command or script found.'"
        ...
        with open(sh_script_path, 'w') as f:
            ...
            if is_explicitly_containerized or auto_sif_path or auto_docker_image:
                ...
                f.write("# The environment is pre-configured, so no conda activation is needed here.\n\n")
                ...
                f.write(f"# The environment is expected to be at: {env_install_path}\n\n")
                f.write(f"conda run --prefix {env_install_path} --no-capture-output bash -c $'\\\n{core_command}\n'\n")

            elif conda_env_spec:
                # Manual setup path (auto-setup not enabled or env not a file)
                f.write("# This job uses a Conda environment. The command is wrapped with 'conda run'.\n")
                f.write("# CRITICAL: You must ensure the environment exists on the execute node.\n")
                f.write("# To enable automatic environment creation, use --auto-conda-setup and --conda-prefix.\n\n")

                if Path(conda_env_spec).is_file() and (conda_env_spec.endswith(".yaml") or conda_env_spec.endswith(".yml")):
                    f.write(f"# The environment is defined by: {conda_env_spec}\n")
                    f.write("# You must create this environment and provide its path.\n")
                    f.write("# Example: conda env create -f {conda_env_spec} -p ./my_conda_envs/{job_details['rule_name']}_env\n")
                    f.write("\n# snake2dagman is assuming you have a pre-existing environment and will attempt to run with it.\n")
                    f.write("# For automatic creation, use --auto-conda-setup and --conda-prefix\n")
                    f.write(f"conda run --prefix {conda_env_spec} --no-capture-output bash -c $'\\\n{core_command}\n'\n")

                else: # Assumed to be a named environment
                    f.write(f"# The environment is specified by name: {conda_env_spec}\n")
                    f.write("# This named environment must be available on the Condor execute nodes.\n")
                    f.write("\n# Using 'conda run' with the specified environment name.\n")
                    # Use bash -c to properly handle complex commands
                    f.write(f"conda run -n {conda_env_spec} --no-capture-output bash -c $'\\\n{core_command}\n'\n")
            ...
            f.write(f"echo 'Job UID {job_uid} (Rule: {job_details['rule_name']}) completed successfully'\n")
...
def generate_run_block_script(script_path, job_uid, job_details, run_code, config_dict, debug=False):
    ...
    for obj_name in ['input', 'output', 'log']:
        ...
        if items:
            ...
            try:
                ...
                preamble.append(f"# WARNING: Could not create namedtuple for '{obj_name}'. Falling back to list.")
    ...
    if resources_dict:
        public_resources = {k: v for k, v in resources_dict.items() if not k.startswith('_') and k.isidentifier()}
        if public_resources:
            sorted_keys = sorted(public_resources.keys())
            Resources = namedtuple('Resources', sorted_keys)
            resources_instance = Resources(**{k: public_resources[k] for k in sorted_keys})
            preamble.append(f"snakemake.resources = {resources_instance!r}")
        else:
            preamble.append("snakemake.resources = None # No public, valid-identifier resources found")
    else:
    ...
    if params_dict:
        ...
        if invalid_keys:
             preamble.append(f"# WARNING: The following keys in 'params' are not valid Python identifiers and have been skipped for dot-notation access: {invalid_keys}")
        ...
        if valid_keys:
            ...
            try:
                ...
                preamble.append("# WARNING: Could not create dot-accessible `params` object. Falling back to a dictionary.")
...
def write_condor_dag(dag_info, output_file, script_paths, workdir, default_memory, default_disk, default_cpus, config, verbose=False, debug=False):
    """Write the Condor DAG file based on job-centric dag_info."""
    
    if not dag_info or 'jobs' not in dag_info:
        print("ERROR: No job information found in dag_info. Cannot write Condor DAG file.")
        return

    if verbose:
        print(f"INFO: Writing Condor DAG file to {output_file}")
        print(f"  Jobs to write: {len(dag_info['jobs'])}")
        print(f"  Default resources: {default_memory} memory, {default_disk} disk, {default_cpus} CPUs")
    ...
    with open(output_file, 'w') as f:
        f.write("# Condor DAG file generated from Snakemake workflow using snake2dagman\n")
        ...
        if conda_envs:
            ...
            auto_docker_build_enabled = any(env.get('docker_image_url') for env in conda_envs.values())
            ...
            if not auto_docker_build_enabled:
                if verbose:
                    print(f"INFO: Writing {len(conda_envs)} conda environment setup jobs")
                for env_info in conda_envs.values():
                    setup_job_name = env_info['setup_job_name']
                    ...
                    f.write("    # This job should be lightweight, but give it some reasonable resources\n")
                    f.write("    request_memory = 1GB\n")
                    f.write("    request_disk = 5GB\n")
                    f.write("}\n\n")
                    
                    if debug:
                        print(f"DEBUG: Wrote setup job description for {setup_job_name}")
            else:
                f.write("# Auto-Docker build is enabled; conda setup jobs are not needed for containerized jobs.\n\n")
        ...
        for job_uid, job_details in dag_info['jobs'].items():
            ...
            if debug:
                ...
                print(f"DEBUG: Job details: {json.dumps(job_details, indent=2, default=str)}")
            ...
            cpu_req = job_details['resources'].cpu if job_details['resources'].cpu > 1 else default_cpus
            ...
            if debug:
                print(f"DEBUG: Resource requests for {condor_job_name}: {mem_req} memory, {disk_req} disk, {cpu_req} CPUs, {gpu_request} GPUs")

            submit_desc_name = f"{condor_job_name}_desc" # Unique name for submit description
            
            f.write(f"# Submit description for Snakemake job UID: {job_uid} (Rule: {job_details['rule_name']})\n")
            f.write(f"SUBMIT-DESCRIPTION {submit_desc_name} {{\n")
            ...
            container_img_path = job_details.get('container_img_path') # This is for pre-converted SIFs
            conda_env_spec = job_details.get('conda_env_spec')
            ...
            if is_containerized and container_img_url:
                f.write("    # This job is containerized using an explicit directive from the Snakefile.\n")
                if "docker://" in container_img_url:
                    ...
                    if container_img_path:
                        ...
                        f.write(f"    # WARNING: Singularity image path not found for URL: {container_img_url}\n")
                        f.write(f"    # +SingularityImage = \"/path/to/your/image.sif\" # PLEASE SPECIFY\n")
            elif auto_sif_path:
                f.write("    # This job is containerized using an auto-converted Apptainer/Singularity image.\n")
                f.write("    universe = vanilla\n")
                f.write(f"    +SingularityImage = \"{auto_sif_path}\"\n")
            elif auto_docker_image:
                f.write("    # This job is containerized using an auto-built Docker image.\n")
                f.write("    universe = docker\n")
            ...
            if script_to_run:
                ...
                f.write(f"    executable = /bin/echo # ERROR: Script not found for {job_uid}\n")
                f.write(f"    arguments = \"Error: Executable script for job UID {job_uid} was not generated.\"\n")
                if debug:
            ...
            f.write("    # Ensure all necessary input files, scripts, and potentially conda envs (if not on shared FS) are transferred.\n")
            f.write("}\n\n")
        ...
        for job_uid, job_details in dag_info['jobs'].items():
            if job_details.get('retries', 0) > 0:
                ...
                if debug:
                    print(f"DEBUG: Added retry directive for {condor_job_name}: {retries} retries")
                ...
                for job_uid, job_details in dag_info['jobs'].items():
                    ...
                    if env_spec:
                        ...
                        if env_path in conda_envs:
                            ...
                            if debug:
                                print(f"DEBUG: Added conda dependency: {setup_job_name} -> {child_job_name}")
                if verbose:
                    print(f"INFO: Wrote {conda_deps_written} conda environment dependencies")
            f.write("\n")
        ...
        if 'job_dependencies' in dag_info and dag_info['job_dependencies']:
                ...
                if dep_uids:
                    ...
                    for parent_uid in dep_uids:
                        if parent_uid in dag_info['jobs']:
                            parent_condor_job_name = dag_info['jobs'][parent_uid]['condor_job_name']
                            f.write(f"PARENT {parent_condor_job_name} CHILD {child_condor_job_name}\n")
                            any_deps_written = True
                            workflow_deps_written += 1
                            if debug:
                                print(f"DEBUG: Added workflow dependency: {parent_condor_job_name} -> {child_condor_job_name}")
            ...
            f.write("# No dependency information found - all jobs may run in parallel if not constrained by resources.\n")
            f.write("# WARNING: This may not be correct - review your Snakemake workflow and the generated DAG.\n")
        
        if verbose:
            print(f"INFO: Successfully wrote {len(dag_info['jobs'])} job definitions to {output_file}")
            
        if debug:
            print(f"DEBUG: DAG file writing complete. File size: {Path(output_file).stat().st_size} bytes")
...
def print_conversion_warnings(dag_info, script_paths, verbose=False, debug=False):
    ...
    for job_uid, job_details in dag_info['jobs'].items():
        ...
        if job_details.get('shell_command') and job_details.get('is_shell'): # Ensure it's an actual shell rule
            shell_rules_info[rule_name].append(job_uid) # Just note the rule has shell jobs
        ...
        if job_details.get('has_dynamic_input') or job_details.get('has_dynamic_output'):
            dynamic_rules.add(rule_name)
        if job_details.get('has_pipe_output'):
            pipe_rules.add(rule_name)
    
    if debug:
        print(f"DEBUG: Warning analysis results:")
    ...
    if has_auto_conda_setup:
        print("   → AUTOMATIC SETUP ENABLED: Conda environments will be created by dedicated setup jobs.")
        print("   → The `--conda-prefix` directory MUST be on a shared filesystem accessible to all nodes.")
        print("   → Jobs have been made children of their corresponding environment setup job.")
        if verbose:
            conda_envs = dag_info.get('conda_envs', {})
            print(f"   → {len(conda_envs)} unique conda environments will be automatically set up.")
        ...
        print("   → MANUAL SETUP REQUIRED: You must ensure conda environments are activated correctly.")
    ...
    print("   → For site-specific attributes (e.g., `+WantGPULab`), add a 'condor_attributes' section")
    print("     to your `.wf2wf.json` file. These are applied to all jobs.")
    print("   → Some resource keywords are not automatically translated to Condor equivalents.")

    print("\n4. CONTAINERIZATION:")
    auto_sif_enabled = any(env.get('apptainer_sif_path') for env in dag_info.get('conda_envs', {}).values())
    auto_docker_enabled = any(env.get('docker_image_url') for env in dag_info.get('conda_envs', {}).values())
    
    if auto_docker_enabled:
        print("   → Auto-Docker build is enabled: Docker images will be built and pushed.")
        if auto_sif_enabled:
            print("   → Apptainer/Singularity images will also be created from Docker images.")
    else:
        print("   → Auto-Docker build is NOT enabled: Docker images will not be built automatically.")
    
    if verbose:
        containerized_jobs = sum(1 for job in dag_info['jobs'].values() if job.get('is_containerized'))
...
def _workflow_to_dag_info(wf: Workflow) -> Dict[str, Any]:
    ...
    for idx, (tid, task) in enumerate(wf.tasks.items()):
        ...
        resources = task.resources if isinstance(task.resources, ResourceSpec) else ResourceSpec()
        env = task.environment if isinstance(task.environment, EnvironmentSpec) else EnvironmentSpec()
    ...
    *,
    ...
    for task in wf.tasks.values():
        ...
        if inline_submit:
            ...
            submit_content = _generate_submit_content(task, script_path, workdir, 
                                                    default_memory, default_disk, default_cpus)
    ...
    if not inline_submit:
        for task in wf.tasks.values():
            ...
            _write_submit_file(task, submit_file, script_paths[task.id], workdir, 
                              default_memory, default_disk, default_cpus)
...
def _write_submit_file(
    ...
    lines = _generate_submit_content(task, script_path, workdir, 
                                   default_memory, default_disk, default_cpus)
...
def _generate_submit_content(
    ...
    resources = task.resources if isinstance(task.resources, ResourceSpec) else ResourceSpec()
    environment = task.environment if isinstance(task.environment, EnvironmentSpec) else EnvironmentSpec()
