...
from wf2wf.core import (
    Workflow, Task, ResourceSpec, EnvironmentSpec,
    ParameterSpec, ScatterSpec, RequirementSpec, ProvenanceSpec,
    DocumentationSpec, BCOSpec
)
...
def from_workflow(wf: Workflow, out_file: str | Path, **opts: Any) -> None:
    ...
    if verbose:
        print(f"Exporting workflow '{wf.name}' to CWL {cwl_version} with enhanced features")
    ...
    try:
        ...
        if use_graph:
            ...
            tool_docs = {}
            for task in wf.tasks.values():
                t_doc = _generate_tool_document_enhanced(task, preserve_metadata=preserve_metadata, structure_prov=structure_prov)
            ...
            wf_doc = _generate_workflow_document_enhanced(
                wf, {tid: f"#{tid}" for tid in wf.tasks}, "", cwl_version,
                preserve_metadata=preserve_metadata, verbose=verbose, structure_prov=structure_prov
            )
        ...
        if single_file:
            ...
            cwl_doc = _generate_single_file_workflow_enhanced(
                wf, cwl_version, preserve_metadata=preserve_metadata, verbose=verbose, structure_prov=structure_prov
            )
        else:
            ...
            tool_refs = _generate_tool_files_enhanced(
                wf, tools_path, output_format, preserve_metadata=preserve_metadata, verbose=verbose, structure_prov=structure_prov
            )
            
            # Generate main workflow with enhanced features
            cwl_doc = _generate_workflow_document_enhanced(
                wf, tool_refs, tools_dir, cwl_version, 
                preserve_metadata=preserve_metadata, verbose=verbose, structure_prov=structure_prov
            )
...
def _generate_workflow_document_enhanced(wf: Workflow, tool_refs: Dict[str, str], 
                                        tools_dir: str, cwl_version: str, 
                                        preserve_metadata: bool = True,
                                        verbose: bool = False, *, structure_prov: bool = False) -> Dict[str, Any]:
    ...
    needs_step_input_expr = any(
        task.meta and 'step_inputs' in task.meta 
        for task in wf.tasks.values()
    )
...
def _generate_workflow_steps_enhanced(wf: Workflow, tool_refs: Dict[str, str], 
                                   tools_dir: str, preserve_metadata: bool = True,
    ...
    for task in wf.tasks.values():
        ...
        if task.retry:
            loss_record(f"/tasks/{task.id}/retry", "retry", task.retry, "Retry count not supported in CWL", "user")
        if task.priority:
            loss_record(f"/tasks/{task.id}/priority", "priority", task.priority, "Priority not representable in CWL", "user")

        # Scatter handling
        if task.scatter:
            scat_list = task.scatter.scatter
            if len(scat_list) == 1 and (task.scatter.scatter_method in (None, '', 'dotproduct')):
                # Shorthand scalar form
                step_def['scatter'] = scat_list[0]
            else:
                step_def['scatter'] = scat_list
                if task.scatter.scatter_method and task.scatter.scatter_method != 'dotproduct':
...
def _generate_tool_files_enhanced(wf: Workflow, tools_path: Path, output_format: str, 
                                preserve_metadata: bool = True, verbose: bool = False, *, structure_prov: bool = False) -> Dict[str, str]:
    ...
    for task in wf.tasks.values():
        ...
        tool_doc = _generate_tool_document_enhanced(task, preserve_metadata=preserve_metadata, structure_prov=structure_prov)
...
def _generate_tool_document_enhanced(task: Task, *, preserve_metadata: bool = True, structure_prov: bool = False) -> Dict[str, Any]:
    ...
    hints = [_requirement_spec_to_cwl(hint) for hint in task.hints] if task.hints else []

    # Auto-inject wf2wf SBOM / SIF provenance as generic hints so they survive round-trip
    sbom_path = task.environment.env_vars.get('WF2WF_SBOM') if task.environment else None
    sif_path = task.environment.env_vars.get('WF2WF_SIF') if task.environment else None

    if sbom_path:
        hints.append({
            'class': 'wf2wf:SBOM',
            'location': str(sbom_path)
        })
...
def _generate_resource_requirement(resources: ResourceSpec) -> Optional[Dict[str, Any]]:
    ...
    if resources.gpu:
        loss_record("/resources/gpu", "gpu", resources.gpu, "GPU resource specification is not representable in CWL", "user")
    if resources.gpu_mem_mb:
        loss_record("/resources/gpu_mem_mb", "gpu_mem_mb", resources.gpu_mem_mb, "GPU memory specification is not representable in CWL", "user")
    if resources.extra:
        loss_record("/resources/extra", "extra", resources.extra, "Extra HTCondor resource attributes dropped in CWL export", "user")
...
def _generate_single_file_workflow_enhanced(wf: Workflow, cwl_version: str, 
                                         preserve_metadata: bool = True, verbose: bool = False, *, structure_prov: bool = False) -> Dict[str, Any]:
    ...
    for step_id, step_def in cwl_doc['steps'].items():
        ...
        step_def['run'] = _generate_tool_document_enhanced(task, preserve_metadata=preserve_metadata, structure_prov=structure_prov)
    
    return cwl_doc


def _write_cwl_document(doc: Dict[str, Any], output_path: Path, 
...
def _add_provenance_to_doc(cwl_doc: Dict[str, Any], provenance: ProvenanceSpec, *, structure: bool = False) -> None:
...
def _add_documentation_to_doc(cwl_doc: Dict[str, Any], documentation: DocumentationSpec) -> None:
...
def _parameter_spec_to_cwl(param_spec: ParameterSpec) -> Dict[str, Any]:
    ...
    def _type_to_cwl(ts):
        ...
        if isinstance(ts, TypeSpec):
            ...
            base = _type_to_cwl(ts.type) if isinstance(ts.type, TypeSpec) else ts.type
            t_repr: Any
            if ts.type == 'array':
                t_repr = {
                    'type': 'array',
...
def _export_bco_document(bco_spec: BCOSpec, bco_path: Path, verbose: bool = False) -> None:
