...
if 'wf2wf' not in sys.modules:
    spec = importlib.util.spec_from_file_location('wf2wf', proj_root / 'wf2wf' / '__init__.py')
    module = importlib.util.module_from_spec(spec)
    sys.modules['wf2wf'] = module  # type: ignore[assignment]
    assert spec and spec.loader
    spec.loader.exec_module(module)  # type: ignore[arg-type]

from wf2wf.core import Workflow, Task, ResourceSpec, EnvironmentSpec, Edge, ParameterSpec

try:
    from wf2wf.cli import (
        detect_input_format, detect_output_format, 
        load_workflow_from_json_yaml, save_workflow_to_json_yaml,
        get_importer, get_exporter
    )
    CLI_AVAILABLE = True
except ImportError:
    try:
...
class TestWorkflowSerialization:
    ...
    def test_save_load_json_roundtrip(self, tmp_path):
        ...
        task1 = Task(id="task1", command="echo 'hello'", outputs=[ParameterSpec(id="file1.txt", type="File")])
        task2 = Task(id="task2", command="echo 'world'", inputs=[ParameterSpec(id="file1.txt", type="File")], outputs=[ParameterSpec(id="file2.txt", type="File")])
        wf.add_task(task1)
        wf.add_task(task2)
...
class TestCleanup:
    ...
    def test_no_base_directory_pollution(self, project_root):
        ...
        if found_dirs:
            ...
            pytest.fail(f"Found unwanted test directories in base directory:\n{dir_list}")
