...
from wf2wf.core import (
    Workflow, Task, ResourceSpec, EnvironmentSpec, ProvenanceSpec, 
    DocumentationSpec, ParameterSpec, RequirementSpec, BCOSpec, ScatterSpec
)
...
class TestCWLFidelityPreservation:
    ...
    def test_requirements_and_hints_preservation(self, persistent_test_output):
        ...
        requirements = [
            RequirementSpec("DockerRequirement", {"dockerPull": "biocontainers/fastqc:v0.11.9_cv8"}),
            RequirementSpec("ResourceRequirement", {
                "coresMin": 4,
                "ramMin": 8192,
                "tmpdirMin": 10240,
                "outdirMin": 5120
            }),
            RequirementSpec("EnvironmentVarRequirement", {
                "envDef": [
        ...
        # Verify requirements
        docker_req = next(r for r in imported_task.requirements if r.class_name == "DockerRequirement")
        assert docker_req.data["dockerPull"] == "biocontainers/fastqc:v0.11.9_cv8"
        
        resource_req = next(r for r in imported_task.requirements if r.class_name == "ResourceRequirement")
        assert resource_req.data["coresMin"] == 4
        assert resource_req.data["ramMin"] == 8192
        
        # Verify hints
        network_hint = next(h for h in imported_task.hints if h.class_name == "NetworkAccess")
...
class TestRoundTripFidelity:
    ...
    def test_cwl_to_dagman_to_cwl_roundtrip(self, persistent_test_output):
        ...
        dagman_from_workflow(imported_workflow, dag_file, scripts_dir=persistent_test_output / "scripts")
    ...
    def _create_comprehensive_cwl_workflow(self) -> Workflow:
        ...
        provenance = ProvenanceSpec(
            authors=["orcid:0000-0000-0000-0002"],
            version="1.5.0",
            license="GPL-3.0"
        )
