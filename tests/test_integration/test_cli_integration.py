...
class TestConfigurationFileHandling:
    ...
    def test_workflow_config_serialization(self):
        ...
        assert wf_dict["config"]["custom_attributes"]["requirements"] == "(OpSysAndVer == \"CentOS7\")"
        
        # Reconstruct workflow
        wf_restored = Workflow.from_json(wf_json)
        assert wf_restored.config["memory_default"] == "16GB"
        assert wf_restored.config["custom_attributes"]["requirements"] == "(OpSysAndVer == \"CentOS7\")"
...
class TestWorkflowValidationAndErrorHandling:
    ...
    def test_invalid_json_config_handling(self):
        ...
        invalid_config = '{"default_memory": "8GB", "default_cpus": 4'  # Missing closing brace
    ...
    def test_resource_validation_edge_cases(self):
        ...
        task_large = Task(
            id="large_resources",
            command="echo 'large'",
            resources=ResourceSpec(cpu=128, mem_mb=1048576, disk_mb=10485760)  # 1TB RAM, 10TB disk
        )
