...
class TestCircularDependencies:
    ...
    def test_circular_dependency_detection_simple(self, tmp_path):
        ...
        def _mock_run_circular(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
            if "--dag" in cmd:
        ...
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_circular):
    ...
    def test_circular_dependency_detection_complex(self, tmp_path):
        ...
        def _mock_run_complex_circular(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
            if "--dag" in cmd:
                m.stdout = ""
                m.stderr = "Error: Circular dependency detected in workflow: A → B → C → A"
                m.returncode = 1
                raise CalledProcessError(1, cmd, output="", stderr=m.stderr)
            return m
        
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_complex_circular):
                with pytest.raises(RuntimeError) as exc_info:
                    snake_importer.to_workflow(snakefile, workdir=tmp_path)
    ...
    def test_workflow_with_cycle_export_fails_gracefully(self):
        ...
        task_c = Task(id="task_c", command="echo C", inputs=["b.txt"], outputs=["c.txt"])
...
class TestEmptyWorkflows:
    ...
    def test_empty_snakefile(self, tmp_path):
        ...
        def _mock_run_empty(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
            if "--dag" in cmd:
        ...
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_empty):
                with pytest.raises(RuntimeError) as exc_info:
                    snake_importer.to_workflow(snakefile, workdir=tmp_path)
    ...
    def test_snakefile_with_only_comments(self, tmp_path):
        ...
        def _mock_run_comments_only(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
            if "--dag" in cmd:
        ...
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_comments_only):
                with pytest.raises(RuntimeError) as exc_info:
                    snake_importer.to_workflow(snakefile, workdir=tmp_path)
    ...
    def test_snakefile_with_no_target_rules(self, tmp_path):
        ...
        def _mock_run_no_targets(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
            if "--dag" in cmd:
        ...
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_no_targets):
                with pytest.raises(RuntimeError) as exc_info:
                    snake_importer.to_workflow(snakefile, workdir=tmp_path)
...
class TestMalformedSnakefiles:
    ...
    def test_python_syntax_error(self, tmp_path):
        ...
        def _mock_run_syntax_error(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
            if "--dag" in cmd:
                m.stdout = ""
        ...
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_syntax_error):
                with pytest.raises(RuntimeError) as exc_info:
                    snake_importer.to_workflow(snakefile, workdir=tmp_path)
    ...
    def test_missing_required_directive(self, tmp_path):
        ...
        def _mock_run_missing_directive(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
            if "--dag" in cmd:
                m.stdout = ""
        ...
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_missing_directive):
                with pytest.raises(RuntimeError) as exc_info:
                    snake_importer.to_workflow(snakefile, workdir=tmp_path)
...
class TestMissingInputFiles:
    ...
    def test_missing_input_files_strict_mode(self, tmp_path):
        ...
        def _mock_run_missing_inputs(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
            if "--dag" in cmd:
                m.stdout = ""
        ...
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_missing_inputs):
                with pytest.raises(RuntimeError) as exc_info:
                    snake_importer.to_workflow(snakefile, workdir=tmp_path)
    ...
    def test_missing_input_files_forceall_mode(self, tmp_path):
        ...
        def _mock_run_forceall(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
...
Nothing to be done."""
        ...
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_forceall):
...
class TestInvalidResourceSpecifications:
    ...
    def test_invalid_resource_syntax(self, tmp_path):
        ...
        def _mock_run_invalid_resources(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
            if "--dag" in cmd:
                m.stdout = ""
        ...
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_invalid_resources):
                with pytest.raises(RuntimeError) as exc_info:
                    snake_importer.to_workflow(snakefile, workdir=tmp_path)
    ...
    def test_negative_resource_values(self, tmp_path):
        ...
        def _mock_run_negative_resources(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
            if "--dag" in cmd:
                m.stdout = """digraph snakemake_dag {
0 [label="rule process"];
...
Nothing to be done."""
        ...
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_negative_resources):
                ...
                assert (process_task.resources.mem_mb == -1000 or 
                       process_task.resources.mem_mb == 0 or  # Default if negative not supported
                       -1000 in process_task.resources.extra.values())
                assert (process_task.resources.cpu == -2 or 
                       process_task.resources.cpu == 1 or  # Default if negative not supported
                       -2 in process_task.resources.extra.values())
...
class TestSnakemakeExecutableHandling:
    ...
    def test_snakemake_permission_denied(self, tmp_path):
        ...
        def _mock_run_permission_denied(cmd, capture_output=False, text=False, check=False, **kwargs):
            raise PermissionError("Permission denied")
        
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_permission_denied):
...
class TestIntegrationErrorHandling:
    ...
    def test_error_propagation_through_pipeline(self, tmp_path):
        ...
        def _mock_run_circular_integration(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
            if "--dag" in cmd:
                m.stdout = ""
        ...
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_circular_integration):
                # Import should fail
    ...
    def test_partial_workflow_recovery(self, tmp_path):
        ...
        def _mock_run_partial_success(cmd, capture_output=False, text=False, check=False, **kwargs):
            m = MagicMock()
            if "--dag" in cmd:
                m.stdout = """digraph snakemake_dag {
...
Nothing to be done."""
        ...
        with patch("wf2wf.importers.snakemake.shutil.which", lambda x: "/usr/bin/snakemake"):
            with patch("wf2wf.importers.snakemake.subprocess.run", side_effect=_mock_run_partial_success):
                ...
                assert ("JOB rule_good_rule_0" in dag_content or 
                       "JOB good_rule" in dag_content or
                       "good_rule" in dag_content)
