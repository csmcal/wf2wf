...
class TestPerformanceAndScaling:
    ...
    def test_complex_dependency_network(self):
        ...
        for task_id in ["A", "B", "C", "D"]:
            task = Task(
                id=task_id,
                command=f"echo 'Task {task_id}' > {task_id}.txt"
            )
            wf.add_task(task)
            tasks.append(task)
        ...
        for i in range(1, 6):
            ...
            task = Task(
                id=task_id,
                command=f"echo 'Fan-out task {i}' > {task_id}.txt"
            )
            wf.add_task(task)
...
class TestEdgeCases:
    ...
    def test_workflow_json_roundtrip_with_complex_data(self, tmp_path):
        ...
        assert loaded_task.params["nested_dict"]["level1"]["level2"] == ["item1", "item2", "item3"]
        assert loaded_task.params["nested_dict"]["level1"]["numbers"] == [1, 2, 3.14, -5]
