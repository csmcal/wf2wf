...
class TestLargeWorkflowHandling:
        ...
        for i in range(num_tasks):
            task = Task(
                ...
                resources=ResourceSpec(
                    ...
                    mem_mb=random.randint(1024, 16384)
                )
            )
            wf.add_task(task)
    ...
    def test_workflow_with_special_characters(self):
        ...
        task2 = Task(
            ...
            command="bash quality-check.sh --threshold=0.95"
        )
        wf.add_task(task2)
        ...
        task3 = Task(
            id="FinalAnalysis",
            command="Rscript FinalAnalysis.R"
        )
        wf.add_task(task3)
...
class TestEdgeCaseHandling:
    ...
    def test_self_dependency_prevention(self):
        ...
        try:
            ...
            self_edges = [e for e in wf.edges if e.parent == "self_task" and e.child == "self_task"]
    ...
    def test_workflow_json_roundtrip_with_complex_data(self):
        ...
        assert wf_restored.config["unicode_string"] == "Test with émojis 🚀 and ñ characters"
        ...
        assert restored_task.resources.extra["requirements"] == "(OpSysAndVer == \"CentOS7\")"
    ...
    def test_workflow_with_unicode_content(self):
        ...
        script_task = Task(
            id="script_with_unicode",
            script="scripts/análisis.py"
        )
        wf.add_task(script_task)
...
class TestDAGManExportPerformance:
        ...
        for i in range(num_tasks):
            task = Task(
                ...
                resources=ResourceSpec(
                    ...
                    mem_mb=random.randint(2048, 8192)
                )
            )
            wf.add_task(task)
    ...
    def test_dagman_export_with_special_characters(self, tmp_path):
        ...
        assert "requirements = (Memory > 4000) && (OpSysAndVer == \"CentOS7\")" in submit_content
