...
class TestNextflowExporter:
    ...
    def test_export_with_retry_and_error_handling(self, persistent_test_output):
        ...
        task1 = Task(id="reliable_task", command="python reliable.py", retry=0)

        task2 = Task(id="flaky_task", command="python flaky.py", retry=3)
    ...
    def test_export_complex_dependencies(self, persistent_test_output):
        ...
        task_c = Task(
            id="task_c", command="echo C", inputs=["a.txt"], outputs=["c.txt"]
        )
        task_d = Task(
            id="task_d", command="echo D", inputs=["b.txt", "c.txt"], outputs=["d.txt"]
        )
        ...
        assert (
            "TASK_D(TASK_B.out, TASK_C.out)" in content
            or "TASK_D(TASK_C.out, TASK_B.out)" in content
        )
    ...
    def test_time_unit_conversion(self, persistent_test_output):
        ...
        task_minutes = Task(
            id="task_min", resources=ResourceSpec(time_s=1800)
        )  # 30 minutes
        task_hours = Task(
            id="task_hour", resources=ResourceSpec(time_s=7200)
        )  # 2 hours
        task_long = Task(
            id="task_long", resources=ResourceSpec(time_s=28800)
        )  # 8 hours
