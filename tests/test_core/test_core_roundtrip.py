...
def minimal_workflow() -> Workflow:
    ...
    wf.add_task(Task(id="step_a", command="echo A", outputs=[ParameterSpec(id="a.txt", type="File")]))
    wf.add_task(Task(id="step_b", command="echo B", inputs=[ParameterSpec(id="a.txt", type="File")], outputs=[ParameterSpec(id="b.txt", type="File")], resources=ResourceSpec(cpu=2, mem_mb=512)))
