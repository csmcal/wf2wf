...
def test_secondary_files_deep():
    p = ParameterSpec(id="sample", type="File", secondary_files=["^.bai", ".tbi", "foo/*/bar"])
    assert len(p.secondary_files) == 3


def test_requirement_validation():
    good_docker = RequirementSpec(class_name="DockerRequirement", data={"dockerPull": "ubuntu:20.04"})
