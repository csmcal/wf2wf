...
class TestContainersConda:
    ...
    def test_container_priority_snakemake_parsing(self, tmp_path):
        ...
        try:
            ...
            assert container_submit_content is not None, "Docker universe not found in any submit file"
...
class TestRunBlocksAndScripts:
    ...
    def test_run_block_snakemake_parsing(self, tmp_path):
        ...
        try:
            ...
            assert "print(f\"Got greeting: {params.greeting}\")" in run_task.meta['run_block_code']
...
class TestRetryAndPriority:
    ...
    def test_dagman_exporter_priority_classad(self, tmp_path):
        ...
        dag_exporter.from_workflow(wf, dag_path, workdir=tmp_path, scripts_dir=scripts_dir)
        ...
        lines_with_priority = [line for line in dag_content.split('\n') if 'PRIORITY' in line and 'normal_priority' in line]
        assert len(lines_with_priority) == 0  # Normal priority task should not have PRIORITY directive
    ...
    def test_snakemake_retries_parsing(self, tmp_path):
        ...
        try:
            ...
            no_retry_task_name = dag_exporter._sanitize_condor_job_name(no_retry_task.id)
...
class TestIntegrationWorkflows:
    ...
    def test_container_priority_workflow_end_to_end(self, tmp_path):
        ...
        try:
            ...
            scripts_dir = tmp_path / "scripts"
            dag_exporter.from_workflow(wf, dag_path, workdir=tmp_path, scripts_dir=scripts_dir, verbose=True)
            ...
            for submit_file in submit_files:
                content = submit_file.read_text()
                if "universe = docker" in content and "docker_image = python:3.9-slim" in content:
                    docker_found = True
                    break
