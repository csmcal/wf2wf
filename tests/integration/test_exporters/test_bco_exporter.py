...
from wf2wf.exporters.bco import from_workflow
...
def _dummy_workflow() -> Workflow:
    ...
    # Single task
    task = Task(id="step_1")
    task.command.set_for_environment("echo hello > result.txt", "shared_filesystem")
...
def test_bco_with_cwl_package(tmp_path: Path):
    ...
    from_workflow(wf, bco_path, include_cwl=True, package=True)
